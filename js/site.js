...
const findme_map = L.map('findme-map')
  ...
  google = L.tileLayer('http://tiles.linkosi.net/google/{z}/{x}/{y}', {
    attribution: 'Tiles &copy; Google &mdash; Source: Google Maps'
  });
...
const findme_marker = L.marker([32.648, 53.564], {
  draggable: true
}).addTo(findme_map);
...
function getNoteBody() {
  ...
  var note_body = "onosm.osmiran.org یادداشتی از یک کسب‌وکار ارسال کرد:\n";
  if ($("#name").val()) note_body += i18n.t('step2.name') + "=" + $("#name").val() + "\n";
  if ($("#hnumberalt").val()) note_body += i18n.t('step2.housenumber') + "=" + $("#hnumberalt").val() + "\n";
  if ($("#addressalt").val()) note_body += i18n.t('step2.street') + "=" + $("#addressalt").val() + "\n";
  if ($("#city").val()) note_body += i18n.t('step2.city') + "=" + $("#city").val() + "\n";
  if ($("#postcode").val()) note_body += i18n.t('step2.postcode') + "=" + $("#postcode").val() + "\n";
...
$("#collect-data-done").click(function () {
  ...
  $.post('https://api.openstreetmap.org/api/0.6/notes.json', qwarg, function (data) {
    ...
    var osm_address = 'https://openstreetmap.org/?note='
    $("#linkcoords").append('<div class="mt-3 h4"><a href="' + link + '">' + osm_address + noteId + '</a></div>');
  });
});
